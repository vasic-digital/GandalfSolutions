...
echo "=== GandalfSolutions UX End-to-End Flow Challenge ==="
echo "  bin=$BIN_PATH timeout=${TIMEOUT_SEC}s"

if [[ -z "$BIN_PATH" ]] || [[ ! -x "$BIN_PATH" ]]; then
    echo "[1/5] SKIP: GANDALFSOLUTIONS_BIN unset — SKIP-OK: #env-binary-missing"
    echo "=== GandalfSolutions UX Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== GandalfSolutions UX Challenge: PASSED ==="
